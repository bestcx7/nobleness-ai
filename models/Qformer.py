...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # 如果将这个模块实例化为跨注意力模块，
        # 则键和值来自编码器；注意力掩码需要使编码器的填充标记不被关注。
        ...
        if attention_mask is not None:
            # 应用注意力掩码是在BertModel前向()函数中为所有层预先计算的。
            attention_scores = attention_scores + attention_mask

        # 将注意力分数归一化为概率
        attention_probs = nn.Softmax(din=-1)(attention_scores)

        if is_cross_attention and self.save_attention:
            self.save_attention_map(attention_probs)
            attention_probs.register_hook(self.save_attn_gradients)

        # 这实际上舍弃整个注意力词汇进行计算，虽然看起来不寻常，但是摘自Transformer paper
        ...
        ouputs = (
            (context_layer, attention_probs) if ouput_attentions else (context_layer,)
        )

        ouputs = ouputs + (past_key_value,)
        return ouputs
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [(config, i) for i in range(config.num_hidden_layers)]
        )
